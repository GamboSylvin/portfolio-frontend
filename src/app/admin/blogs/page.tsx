...
export default function AdminBlogsPage() {
  ...
  async function fetchBlogs() {
    ...
    try {
      ...
      if (!res.ok) throw new Error((payload as { error?: string }).error || 'Failed to fetch blog posts');
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      const url = editingSlug ? `/api/admin/blog/${editingSlug}` : '/api/admin/blog';
      ...
      const payload = await res.json();
      if (!res.ok) throw new Error((payload as { error?: string }).error || 'Failed to save post');
  ...
  async function handleEdit(item: BlogItem) {
  ...
  async function handleDelete(slug: string) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error((payload as { error?: string }).error || 'Failed to delete blog post');
      }
      await fetchBlogs();
    } catch (err) {
  ...
  if (loading) {
    return <div className="text-slate-400">Loading blog posts...</div>;
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-4 rounded-2xl border border-slate-800 bg-slate-900 p-6">
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <input value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className="w-full rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-white" />
          </div>
        </div>

        <div>
          <label className="mb-2 block text-sm text-slate-300">Description</label>
          <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={3} className="w-full rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-white" required />
        </div>

        <div>
          <label className="mb-2 block text-sm text-slate-300">Tags</label>
          <input value={form.tags} onChange={(e) => setForm({ ...form, tags: e.target.value })} placeholder="nextjs, react, portfolio" className="w-full rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-white" />
        </div>

        <div>
          <label className="mb-2 block text-sm text-slate-300">Content</label>
          <textarea value={form.content} onChange={(e) => setForm({ ...form, content: e.target.value })} rows={8} className="w-full rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-white" required />
        </div>
        ...
        <div className="flex gap-3">
          <button type="submit" disabled={saving} className="rounded-lg bg-cyan-500 px-4 py-2 font-semibold text-slate-950 hover:bg-cyan-400 disabled:opacity-60">
            {saving ? 'Saving...' : editingSlug ? 'Update post' : 'Add post'}
          </button>
          {editingSlug ? (
